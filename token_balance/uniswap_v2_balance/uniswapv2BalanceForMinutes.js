...
export async function getUniswapV2BalanceByMinut(startTimestamp=0,endTimestamp=Date.now()/1000){
    try{
        return fillBigArrayForMinues(reformToBigArrayForMinutes(await getUniswapV2BalanceByMinutesFromGraph()),startTimestamp,endTimestamp)
    }
    catch(err)
    {
        console.log(err)
...
async function getUniswapV2BalanceByMinutesFromGraph(){
    try{
