...
const hourQuery =`
{
    balanceYears(first:1000 orderBy:timestamp where:{token:"0x6B175474E89094C44Da98b954EedeAC495271d0F"}){
...
export async function getDAIBalanceByHour(startTimestamp=0,endTimestamp=Date.now()/1000){
    try{
       
       
        return fillBigArrayForHours(reformToBigArrayForHours( await getDAIBalanceByHoursFromGraph()),startTimestamp,endTimestamp);
    }
    catch(err)
    {
        console.log(err)
...
export async function getDAIBalanceBy4Hour(startTimestamp=0,endTimestamp=Date.now()/1000){
    try{

        return fillBigArrayFor4Hours(reformToBigArrayForHours( await getDAIBalanceByHoursFromGraph()),startTimestamp,endTimestamp);
    }
    catch(err)
    {
        console.log(err)
...
async function getDAIBalanceByHoursFromGraph(){
    try{
