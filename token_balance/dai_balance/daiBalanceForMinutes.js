...
export async function getDAIBalanceByMinut(startTimestamp=0,endTimestamp=Date.now()/1000){
    try{
        return fillBigArrayForMinues(reformToBigArrayForMinutes(await getDAIBalanceByMinutesFromGraph()),startTimestamp,endTimestamp)
    }
    catch(err)
    {
        console.log(err)
...
async function getDAIBalanceByMinutesFromGraph(){
    try{
...
function fillBigArrayForMinues(bigArray,startTimestamp,endTimestamp){
    ...
    console.log(j,bigArray.length)
    console.log(bigArray[j-1].timestamp<startTimestamp)
