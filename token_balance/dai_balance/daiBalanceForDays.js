...
export async function getDAIBalanceByDay(startTimestamp=0,endTimestamp=Date.now()/1000) {
    try{
        return fillBigArrayForDays(reformToBigArrayForDays( await getDAIBalanceByDaysFromGraph()),startTimestamp,endTimestamp);
    }
    catch(err)
    {
        console.log(err)
...
async function getDAIBalanceByDaysFromGraph(){
    try{
