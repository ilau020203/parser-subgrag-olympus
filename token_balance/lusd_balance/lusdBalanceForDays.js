...
export async function getLUSDBalanceByDay(startTimestamp=0,endTimestamp=Date.now()/1000) {
    try{
        return fillBigArrayForDays(reformToBigArrayForDays( await getLUSDBalanceByDaysFromGraph()),startTimestamp,endTimestamp);
    }
    catch(err)
    {
        console.log(err)
...
async function getLUSDBalanceByDaysFromGraph(){
    try{
