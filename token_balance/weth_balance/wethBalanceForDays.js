...
export async function getWETHBalanceByDay(startTimestamp=0,endTimestamp=Date.now()/1000) {
    try{
        return fillBigArrayForDays(reformToBigArrayForDays( await getWETHBalanceByDaysFromGraph()),startTimestamp,endTimestamp);
    }
    catch(err)
    {
        console.log(err)
...
async function getWETHBalanceByDaysFromGraph(){
    try{
